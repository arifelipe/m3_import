...
>
>**3D Print Preparation Feature**: Developed by MickeyKnox#1985
...
This version includes a new feature developed by **MickeyKnox#1985** that helps prepare models for 3D printing using normal maps.

### Features

- **Smart Adaptive Subdivision**: Automatically applies normal map details to geometry
- **Automatic Texture Detection**: Searches for normal maps in materials, supporting multiple naming conventions
- **Vertex Group Masking**: Creates detail masks based on texture alpha channel
- **Multi-Stage Process**:
  1. Vertex weight editing with texture-based masking
  2. Subdivision modifier for detail enhancement
  3. Displacement modifier using normal map
  4. Decimate optimization for areas without details

### How to Use

1. Select a mesh object in Blender
2. Ensure the object has materials with normal map textures loaded
3. Go to the **Properties Editor** → **Material** tab
4. In the **M3 Material Selection** panel, click the **"Prepare for 3D Print"** button (🖨️ icon)
5. The tool will automatically:
   - Search for normal/displacement maps in your materials
   - Create necessary modifiers and vertex groups
   - Apply adaptive subdivision based on texture details
   - Optimize the mesh for printing

### Requirements

- Mesh object selected
- Material with normal map loaded (supports textures named with: "normal", "norm", "spec", or "height")
- Materials must use nodes

### Tips

- If the model looks too "noisy", increase the displacement mid-level or decrease strength
- The process is undoable (Ctrl+Z) for quick adjustments
- Works best with models that have good UV mapping
