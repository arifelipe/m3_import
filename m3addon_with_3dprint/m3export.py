...
class Exporter:
        ...
        if visualizeBoundingBoxes:
            def selectOrCreateBone(scene, boneName, location):
                ...
                for currentBone in armature.bones:
                    is_current = currentBone.name == boneName
                    if hasattr(currentBone, "select"):
                        currentBone.select = is_current
                    if hasattr(currentBone, "select_head"):
                        currentBone.select_head = is_current
                    if hasattr(currentBone, "select_tail"):
                        currentBone.select_tail = is_current
                poseBone = armatureObject.pose.bones[boneName]
    ...
    def findFCurveWithPathAndIndex(self, action, animPath, curveArrayIndex):
        for curve in shared.action_fcurves(action):
        ...
        if animatedPropertyToDefaultValueMap is None:
            ...
            if rootObject.animation_data is not None:
                ...
                if currentAction is not None:
                    for curve in shared.action_fcurves(currentAction):
