...
class Importer:
    ...
    def importM3BasedOnM3ImportOptions(self, scene: bt.Scene):
        ...
        if self.contentPreset == cm.M3ImportContentPreset.Everything:
            ...
            # but only if we are creating a new armature/resetting the session
            if scene.m3_import_options.armatureObject is None:
                self.scene.m3_animation_ids.clear()
            self.storeModelId()
            self.createAnimations()
            
            # Only import visibility test if it's a new model or if it has valid data (m3a often has 0 radius)
            if scene.m3_import_options.armatureObject is None or self.model.boundings.radius > 0:
    ...
    def animateBone(self, boneIndex, m3Bone, leftCorrectionMatrix, rightCorrectionMatrix, defaultLocation, defaultRotation, defaultScale):
        ...
        for animationTempData in self.animations:
            ...
            group = boneName
            fcurves = shared.action_fcurves(action)
            if locationAnimId in animIdToTimeValueMap:
                locXCurve = fcurves.new(locationAnimPath, index=0)
                locYCurve = fcurves.new(locationAnimPath, index=1)
                locZCurve = fcurves.new(locationAnimPath, index=2)
            ...
            if rotationAnimId in animIdToTimeValueMap:
                rotWCurve = fcurves.new(rotationAnimPath, index=0)
                rotXCurve = fcurves.new(rotationAnimPath, index=1)
                rotYCurve = fcurves.new(rotationAnimPath, index=2)
                rotZCurve = fcurves.new(rotationAnimPath, index=3)
                for timeInMS, frame in zip(timeEntries, frames):
                    rotation = timeToRotationMap.get(timeInMS)
            ...
            if scaleAnimId in animIdToTimeValueMap:
                scaXCurve = fcurves.new(scaleAnimPath, index=0)
                scaYCurve = fcurves.new(scaleAnimPath, index=1)
                scaZCurve = fcurves.new(scaleAnimPath, index=2)
                for timeInMS, frame in zip(timeEntries, frames):
                    scale = timeToScaleMap.get(timeInMS)
    ...
    def animateFloat(self, objectWithAnimationData, path, animId, defaultValue):
        ...
        for action, timeValueMap in self.actionAndTimeValueMapPairsFor(animId):
            fcurves = shared.action_fcurves(action)
            curve = fcurves.new(path, index=0)
    ...
    def animateVector3(self, objectWithAnimationData, path, animId, defaultValue):
        ...
        for action, timeValueMap in self.actionAndTimeValueMapPairsFor(animId):
            fcurves = shared.action_fcurves(action)
            xCurve = fcurves.new(path, index=0)
            yCurve = fcurves.new(path, index=1)
            zCurve = fcurves.new(path, index=2)
    ...
    def animateColor(self, objectWithAnimationData, path, animId, m3DefaultValue):
        ...
        for action, timeValueMap in self.actionAndTimeValueMapPairsFor(animId):
            fcurves = shared.action_fcurves(action)
            redCurve = fcurves.new(path, index=0)
            greenCurve = fcurves.new(path, index=1)
            blueCurve = fcurves.new(path, index=2)
            alphaCurve = fcurves.new(path, index=3)
    ...
    def animateBoundings(self, objectWithAnimationData, animPathMinBorder, animPathMaxBorder, animPathRadius, animId, minBorderDefault, maxBorderDefault, radiusDefault):
        ...
        for action, timeValueMap in self.actionAndTimeValueMapPairsFor(animId):
            fcurves = shared.action_fcurves(action)
            minXCurve = fcurves.new(animPathMinBorder, index=0)
            minYCurve = fcurves.new(animPathMinBorder, index=1)
            minZCurve = fcurves.new(animPathMinBorder, index=2)
            maxXCurve = fcurves.new(animPathMaxBorder, index=0)
            maxYCurve = fcurves.new(animPathMaxBorder, index=1)
            maxZCurve = fcurves.new(animPathMaxBorder, index=2)
            radiusCurve = fcurves.new(animPathRadius, index=0)
...
def importM3BasedOnM3ImportOptions(scene):
    ...
    return importer.armatureObject
