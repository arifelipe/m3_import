...
def createMaterialForMesh(scene: bt.Scene, mesh: bt.Mesh):
    ...
    mlog.info("--- Generating Blender Material: %s ---" % standardMaterial.name)
    ...
    # Remove old materials and apply the new one
    if len(mesh.materials) == 0:
        mesh.materials.append(realMaterial)
    else:
        # Match material slot name if possible, otherwise use slot 0
        target_slot = 0
        for i, slot in enumerate(mesh.materials):
            if slot and slot.name == realMaterial.name:
                target_slot = i
                break
        mesh.materials[target_slot] = realMaterial
        mlog.info("Assigned material to slot %d" % target_slot)
