...
bl_info = {
    "name": "M3 Addon (Testing/Fixed for HotS + 3D Print)",
    "author": "Florian Köberle, netherh, chaos2night, Talv, Solstice245, MickeyKnox#1985",
    "version": (0, 7, 0),
    "blender": (5, 0, 0),
    "location": "Properties Editor -> Scene -> M3 Panels",
    "description": "Allows to export and import models in M3 format. Includes 3D print preparation, integrated upscaling and asset pipeline compatibility.",
...
from . import cm, shared, ui, geonodes_3dprint
from .common import mlog
from .shared import (
    draw_collection_op_generic,
    m3_ob_getter,
    m3_ob_setter,
    removeBone,
    selectBone,
    selectBoneIfItExists,
    selectOrCreateBone,
)
...
def handleGeometricShapeTypeOrBoneNameUpdate(self, context):
    ...
    shapeObject.name = "{name} ({type})".format(
        name=shapeObject.boneName, type=typeName
    )
...
def handleParticleSystemsVisiblityUpdate(self, context):
    ...
    for particleSystem in scene.m3_particle_systems:
        shared.setBoneVisibility(
            scene, particleSystem.boneName, self.showParticleSystems
        )
...
def handleAttachmentPointVisibilityUpdate(self, context):
    ...
    for attachmentPoint in scene.m3_attachment_points:
        shared.setBoneVisibility(
            scene, attachmentPoint.boneName, self.showAttachmentPoints
        )
...
def handleAnimationChange(ob, animation, prefix=""):
    ...
    if animation:
        ...
        action = (
            bpy.data.actions[animationName]
            if animationName in bpy.data.actions
            else bpy.data.actions.new(prefix + animation.name)
        )
    ...
    if action:
        ...
        # mlog.debug("Applying action %s to %s" % (action.name, ob.name))
        shared.action_fcurves(action, ob)

    # Force update
    if hasattr(ob, "update_tag"):
        ob.update_tag()
...
def handleAnimationSequenceIndexChange(self, context):
    ...
    for ob in scene.objects:
        if ob.type == "ARMATURE":
            ...
            handleAnimationChange(ob, animation, prefix=ob.name)
...
def handleAnimationSequenceNameChange(self, context):
    if "Armature Object" + self.nameOld in bpy.data.actions:
        bpy.data.actions["Armature Object" + self.nameOld].name = (
            "Armature Object" + self.name
        )
...
def prepareDefaultValuesForNewAction(objectWithAnimationData, newAction):
    ...
    propertiesBecomingUnanimated = oldAnimatedProperties.difference(
        newAnimatedProperties
    )

    if len(removedProperties) > 0:
        mlog.debug(
            "Removing animations for %s since those properties do no longer exist"
            % removedProperties
        )

    removedCurves = list()
    if newAction is not None:
        newFcurves = shared.action_fcurves(newAction)
        for curve in newFcurves:
            if (curve.data_path, curve.array_index) in removedProperties:
                removedCurves.append(curve)
        for removedCurve in removedCurves:
            newFcurves.remove(removedCurve)

    defaultsToRemove = set()

    for curve in shared.action_fcurves(defaultAction):
        ...
        if prop in propertiesBecomingUnanimated:
            ...
            if propertyExists:
                ...
                if converter in [float, int, bool]:
                    ...
                    resolvedObject = objectWithAnimationData.path_resolve(
                        curvePath[:dotIndex]
                    )
    ...
    defaultFcurves = shared.action_fcurves(defaultAction)
    for curve in defaultFcurves:
        if (curve.data_path, curve.array_index) in removedProperties:
            removedDefaultCurves.append(curve)
    for removedDefaultCurve in removedDefaultCurves:
        defaultFcurves.remove(removedDefaultCurve)
...
def handleParticleSystemCopyIndexChanged(self, context):
    scene = context.scene
    particleSystem = self
    if particleSystem.copyIndex >= 0 and particleSystem.copyIndex < len(
        particleSystem.copies
    ):
...
emissionAreaTypeList = [
    ...
    (
        shared.emissionAreaTypeMesh,
        "Spline",
        "Spawn location are the vertices of a mesh",
    ),
    (
        "7",
        "Mesh",
        "If Vertex Color is applied and exported by the material on the mesh, then the Red channel of vertex color regulates the probability that a face will be used for particle emission",
    ),
]
...
particleTypeList = [
    ...
    (
        "1",
        "Speed Scaled and Rotated Billbords",
        "Particles are rectangles scaled which get scaled by speed by a configurable amounth",
    ),
    ...
    (
        "6",
        "Rectangular Billbords",
        "Rectangles which can have a length != witdh which are rotated towards the camera",
    ),
    (
        "7",
        "Quads with speed as normal",
        "Particles are quads which have their normals aligned to the speed vector of the particle",
    ),
    ("8", "Unknown (Id 8)", "Code 8 with unknown meaning"),
    (
        "9",
        "Ray from Spawn Location",
        "A billboard that reaches from the spawn location to the current position",
    ),
...
ribbonCullType = [
    (
        "0",
        "Lifespan",
        "Ribbon elements are destroyed after having existed for the specified lifespan.",
    ),
    (
        "1",
        "Length Based",
        "Ribbon elements are destroyed after reaching the specified maximum length",
    ),
]
...
forceTypeList = [
    ...
    (
        "2",
        "Dampening",
        "This is a drag operation that resists the movement of particles",
    ),
    (
        "3",
        "Vortex",
        "This is a special rotation field that brings particles into a orbit. Does not work with Box Shape applied.",
    ),
]
...
forceShapeList = [
    ...
    (
        "4",
        "ConeDome",
        "Special cone shape with length defined as Radius and cone width defined as Angle.",
    ),
]
...
particleEmissionTypeList = [
    (
        "0",
        "Constant",
        "Emitted particles fly towards a configureable direction with a configurable spread",
    ),
    ("1", "Radial", "Particles move into all kinds of directions"),
    (
        "2",
        "Z Axis",
        "Picks randomly to move in the direction of the positive or negative local Z-Axis for the emitter.e",
    ),
    ("3", "Random", "Picks an entirely arbitrary orientation."),
    (
        "4",
        "Mesh Normal",
        "when using a Mesh Emitter Shape, uses the normal of the face being emitted from as the direction vector.",
    ),
]
...
attachmentVolumeTypeList = [
    ...
    (
        shared.attachmentVolumeCuboid,
        "Cuboid",
        "Volume with the shape of a cuboid with the given width, length and height",
    ),
    (
        shared.attachmentVolumeSphere,
        "Sphere",
        "Volume with the shape of a sphere with the given radius",
    ),
    (
        shared.attachmentVolumeCapsule,
        "Capsule",
        "Volume with the shape of a cylinder with the given radius and height",
    ),
...
physicsJointType = [
    ...
    (
        "2",
        "Cone",
        "Allows the joint to move to move freely like a spherical joint, but constrains movement to the cone angle, around the Z axis",
    ),
...
matDefaultSettingsList = [
    ...
    (
        defaultSettingParticle,
        "Particle Standard Material",
        "Material for particle systems",
    ),
    (
        defaultSettingDisplacement,
        "Displacement Material",
        "Moves the colors of the background to other locations",
    ),
    (
    ...
    (
        defaultSettingTerrain,
        "Terrain Material",
        "Makes the object look like the ground below it",
    ),
    ...
    (
        defaultSettingCreep,
        "Creep Material",
        "Looks like creep if there is creep below the model and is invisible otherwise",
    ),
    (defaultSettingSplatTerrainBake, "STB Material", "Splat Terrain Bake Material"),
    (
        defaultSettingLensFlare,
        "Lens Flare Material",
        "Lens flare material which can not be exported yet",
    ),
]
...
billboardBehaviorTypeList = [
    (
        "0",
        "Local X",
        "Bone gets oriented around X towards the camera, but rotates then with the model",
    ),
    (
        "1",
        "Local Z",
        "Bone gets oriented around Z towards the camera, but rotates then with the model",
    ),
    (
        "2",
        "Local Y",
        "Bone gets oriented around Y towards the camera, but rotates then with the model",
    ),
    (
        "3",
        "World X",
        "Bone gets oriented around X towards the camera, independent of model rotation",
    ),
    (
        "4",
        "World Z",
        "Bone gets oriented around Z towards the camera, independent of model rotation",
    ),
    (
        "5",
        "World Y",
        "Bone gets oriented around Y towards the camera, independent of model rotation",
    ),
    (
        "6",
        "World All",
        "Bone orients itself towards the camera and rotates around all axes to do so",
    ),
]
...
class M3TransformationCollection(bpy.types.PropertyGroup):
    ...
    animatedProperties: bpy.props.CollectionProperty(
        options=set(), type=M3AnimatedPropertyReference
    )
...
class M3Animation(bpy.types.PropertyGroup):
    name: bpy.props.StringProperty(
        options=set(), update=handleAnimationSequenceNameChange
    )
    nameOld: bpy.props.StringProperty(options=set())
    startFrame: bpy.props.IntProperty(
        options=set(),
        subtype="UNSIGNED",
        update=handleAnimationSequenceStartFrameChange,
    )
    ...
    exlusiveEndFrame: bpy.props.IntProperty(
        options=set(),
        subtype="UNSIGNED",
        default=60,
        update=handleAnimationSequenceEndFrameChange,
    )
    transformationCollections: bpy.props.CollectionProperty(
        options=set(), type=M3TransformationCollection
    )
...
class M3StandardMaterial(bpy.types.PropertyGroup):
    name: bpy.props.StringProperty(
        options=set(), default="Material", update=handleMaterialNameChange
    )
    ...
    blendMode: bpy.props.EnumProperty(
        options=set(), items=matBlendModeList, default="0"
    )
...
Defines the sorting relationship between this material and other transparent materials in the model file. Within a given model, materials are drawn in order from highest priority value to lowest. Sorting for materials of equal priority is undefined. Priorities have no effect across multiple models.""",
    ...
    layerBlendType: bpy.props.EnumProperty(
        options=set(), items=matLayerAndEmisBlendModeList, default="2"
    )
    emisBlendType: bpy.props.EnumProperty(
        options=set(), items=matLayerAndEmisBlendModeList, default="2"
    )
    emisMode: bpy.props.EnumProperty(
        options=set(), items=matLayerAndEmisBlendModeList, default="2"
    )  # what is the difference between emisBlendType and emisMode?
    specType: bpy.props.EnumProperty(
        options=set(), items=matSpecularTypeList, default="0"
    )
    ...
    depthBlendFalloff: bpy.props.FloatProperty(
        options=set(),
        update=handleDepthBlendFalloffChanged,
        default=0,
        description=f"""\
Extends the distance over which particle effects fade out when they get close to a solid object. To reduce visual clipping artifacts, particle effects in StarCraft II fade out as they get close to another surface. Increasing this value causes the blend to happen over a wider range. Disable Soft (Depth Blend) can bypass this effect when no depth blend is desired.
    ...
    useDepthBlendFalloff: bpy.props.BoolProperty(
        options=set(),
        update=handleUseDepthBlendFalloffChanged,
        description="Should be true for particle system materials",
        default=False,
    )
    useVertexColor: bpy.props.BoolProperty(
        options=set(),
        description="The vertex color layer named color will be used to tint the model",
        default=False,
    )
    useVertexAlpha: bpy.props.BoolProperty(
        options=set(),
        description="The vertex color layer named alpha, will be used to determine the alpha of the model",
        default=False,
    )
...
class M3DisplacementMaterial(bpy.types.PropertyGroup):
    ...
    # the following field gets used to update the name of the material reference:
    materialReferenceIndex: bpy.props.IntProperty(options=set(), default=-1)
    strengthFactor: bpy.props.FloatProperty(
        name="strength factor",
        options={"ANIMATABLE"},
        default=1,
        description="Factor that gets multiplied with the strength values",
    )
    layers: bpy.props.CollectionProperty(options=set(), type=cm.M3MaterialLayer)
...
class M3CompositeMaterialSection(bpy.types.PropertyGroup):
    ...
    alphaFactor: bpy.props.FloatProperty(
        name="alphaFactor",
        options={"ANIMATABLE"},
        min=0,
        max=1,
        default=1,
        description="Defines the factor with which the alpha channel gets multiplied",
    )
...
class M3CompositeMaterial(bpy.types.PropertyGroup):
    ...
    sections: bpy.props.CollectionProperty(
        options=set(), type=M3CompositeMaterialSection
    )
...
class M3VolumeMaterial(bpy.types.PropertyGroup):
    ...
    # the following field gets used to update the name of the material reference:
    materialReferenceIndex: bpy.props.IntProperty(options=set(), default=-1)
    volumeDensity: bpy.props.FloatProperty(
        options={"ANIMATABLE"},
        default=0.3,
        description="Factor that gets multiplied with the strength values",
    )
...
class M3VolumeNoiseMaterial(bpy.types.PropertyGroup):
    ...
    layers: bpy.props.CollectionProperty(options=set(), type=cm.M3MaterialLayer)
    scrollRate: bpy.props.FloatVectorProperty(
        default=(0, 0, 0.8), size=3, subtype="XYZ", options={"ANIMATABLE"}
    )
    translation: bpy.props.FloatVectorProperty(
        default=(0, 0, 0), size=3, subtype="XYZ", options={"ANIMATABLE"}
    )
    scale: bpy.props.FloatVectorProperty(
        default=(2, 2, 1), size=3, subtype="XYZ", options={"ANIMATABLE"}
    )
...
class M3Boundings(bpy.types.PropertyGroup):
    ...
    center: bpy.props.FloatVectorProperty(
        options=set(), default=(0, 0, 0), size=3, subtype="XYZ"
    )
...
class M3ParticleSpawnPoint(bpy.types.PropertyGroup):
    location: bpy.props.FloatVectorProperty(
        ...
        subtype="XYZ",
        options={"ANIMATABLE"},
        description="The first two values are the initial and final size of particles",
    )


class M3ParticleSystemCopy(bpy.types.PropertyGroup):
    name: bpy.props.StringProperty(
        options=set(), update=handleParticleSystemCopyNameChange
    )
    boneName: bpy.props.StringProperty(options=set())
    emissionRate: bpy.props.FloatProperty(default=10, options={"ANIMATABLE"})
    partEmit: bpy.props.IntProperty(
        default=0, subtype="UNSIGNED", options={"ANIMATABLE"}
    )
...
class M3ParticleSystem(bpy.types.PropertyGroup):
    ...
    maxParticles: bpy.props.IntProperty(
        options=set(), default=20, min=0, subtype="UNSIGNED"
    )
    emissionSpeed1: bpy.props.FloatProperty(
        options={"ANIMATABLE"},
        default=0,
        description="The initial speed of the particles at emission",
    )
    emissionSpeed2: bpy.props.FloatProperty(
        default=1,
        options={"ANIMATABLE"},
        description="If emission speed randomization is enabled this value specfies the other end of the range of random speeds",
    )
    randomizeWithEmissionSpeed2: bpy.props.BoolProperty(
        options=set(),
        default=False,
        description="Specifies if the second emission speed value should be used to generate random emission speeds",
    )
    emissionAngleX: bpy.props.FloatProperty(
        default=0,
        subtype="ANGLE",
        options={"ANIMATABLE"},
        description="Specifies the X rotation of the emission vector",
    )
    emissionAngleY: bpy.props.FloatProperty(
        default=0,
        subtype="ANGLE",
        options={"ANIMATABLE"},
        description="Specifies the Y rotation of the emission vector",
    )
    emissionSpreadX: bpy.props.FloatProperty(
        default=0,
        options={"ANIMATABLE"},
        description="Specifies in radian by how much the emission vector can be randomly rotated around the X axis",
    )
    emissionSpreadY: bpy.props.FloatProperty(
        default=0,
        options={"ANIMATABLE"},
        description="Specifies in radian by how much the emission vector can be randomly rotated around the Y axis",
    )
    lifespan1: bpy.props.FloatProperty(
        default=0.5,
        min=0,
        options={"ANIMATABLE"},
        description="Specfies how long it takes before the particles start to decay",
    )
    lifespan2: bpy.props.FloatProperty(
        default=5,
        min=0,
        options={"ANIMATABLE"},
        description="If random lifespans are enabled this specifies the other end of the range for random lifespan values",
    )
    randomizeWithLifespan2: bpy.props.BoolProperty(
        options=set(),
        default=True,
        description="Specifies if particles should have random lifespans",
    )
    killSphere: bpy.props.FloatProperty(
        options=set(),
        default=0,
        min=0,
        description="For non-zero values, any particle which goes outside the specified radius from the system is destroyed",
    )
    zAcceleration: bpy.props.FloatProperty(
        options=set(),
        default=0,
        description="Negative gravity which does not get influenced by the emission vector",
    )
    sizeAnimationMiddle: bpy.props.FloatProperty(
        options=set(),
        default=0.5,
        min=0,
        max=1,
        subtype="FACTOR",
        description="Factor of lifetime when the scale animation reaches its middle value",
    )
    colorAnimationMiddle: bpy.props.FloatProperty(
        options=set(),
        default=0.5,
        min=0,
        max=1,
        subtype="FACTOR",
        description="Factor of lifetime when the color animation reaches its middle value",
    )
    alphaAnimationMiddle: bpy.props.FloatProperty(
        options=set(),
        default=0.5,
        min=0,
        max=1,
        subtype="FACTOR",
        description="Factor of lifetime when the alpha animation reaches its middle value",
    )
    rotationAnimationMiddle: bpy.props.FloatProperty(
        options=set(),
        default=0.5,
        min=0,
        max=1,
    ...
    sizeHoldTime: bpy.props.FloatProperty(
        options=set(),
        default=0.3,
        min=0,
        max=1,
        subtype="FACTOR",
        description="Factor of particle liftime to hold the middle size value",
    )
    colorHoldTime: bpy.props.FloatProperty(
        options=set(),
        default=0.3,
        min=0,
        max=1,
        subtype="FACTOR",
        description="Factor of particle lifetime to hold the middle color value",
    )
    alphaHoldTime: bpy.props.FloatProperty(
        options=set(),
        default=0.3,
        min=0,
        max=1,
        subtype="FACTOR",
        description="Factor of particle lifetime to hold the middle alpha value",
    )
    rotationHoldTime: bpy.props.FloatProperty(
        options=set(),
        default=0.3,
        min=0,
        max=1,
        subtype="FACTOR",
        description="Factor of particle lifetime to hold the middle rotation value",
    )
    sizeSmoothingType: bpy.props.EnumProperty(
        options=set(),
        default="0",
        items=animationSmoothTypeList,
        description="Determines the shape of the size curve based on the intial, middle , final and hold time value",
    )
    colorSmoothingType: bpy.props.EnumProperty(
        options=set(),
        default="0",
        items=animationSmoothTypeList,
        description="Determines the shape of the color curve based on the intial, middle , final and hold time value",
    )
    rotationSmoothingType: bpy.props.EnumProperty(
        options=set(),
        default="0",
        items=animationSmoothTypeList,
        description="Determines the shape of the rotation curve based on the intial, middle , final and hold time value",
    )
    particleSizes1: bpy.props.FloatVectorProperty(
        default=(1, 1, 1),
        size=3,
        subtype="XYZ",
        options={"ANIMATABLE"},
    ...
    rotationValues1: bpy.props.FloatVectorProperty(
        default=(0, 0, 0),
        size=3,
        subtype="XYZ",
        options={"ANIMATABLE"},
        description="The first value is the inital rotation and the second value is the rotation speed",
    )
    initialColor1: bpy.props.FloatVectorProperty(
        default=(1, 1, 1, 1),
        min=0,
        max=1,
        size=4,
        subtype="COLOR",
        options={"ANIMATABLE"},
        description="Color of the particle when it gets emitted",
    )
    middleColor1: bpy.props.FloatVectorProperty(
        default=(1, 1, 1, 1),
        min=0,
        max=1,
        size=4,
    ...
    finalColor1: bpy.props.FloatVectorProperty(
        default=(1, 1, 1, 0.5),
        min=0,
        max=1,
        size=4,
        subtype="COLOR",
        options={"ANIMATABLE"},
        description="The color the particle will have when it vanishes",
    )
    drag: bpy.props.FloatProperty(
        options=set(),
        default=1,
        min=0,
        description="The amounth of speed reduction while the particle is falling, multiplied by the particle's velocity",
    )
    mass: bpy.props.FloatProperty(
        options=set(),
        default=0.001,
        description="Mass determines the effects of drag and the influence of Forces",
    )
    ...
    randomizeWithMass2: bpy.props.BoolProperty(
        options=set(),
        default=True,
        description="Specifies if the second mass value should be used to generate random mass values",
    )
    unknownFloat2c: bpy.props.FloatProperty(options=set(), default=2)
    trailingEnabled: bpy.props.BoolProperty(
        options=set(),
        default=True,
        description="If trailing is enabled then particles don't follow the particle emitter",
    )
    emissionRate: bpy.props.FloatProperty(default=10, min=0, options={"ANIMATABLE"})
    emissionAreaType: bpy.props.EnumProperty(
        options=set(), default="2", items=emissionAreaTypeList
    )
    cutoutEmissionArea: bpy.props.BoolProperty(options=set())
    emissionAreaSize: bpy.props.FloatVectorProperty(
        default=(0.1, 0.1, 0.1),
        update=handleParticleSystemAreaSizeChange,
        size=3,
        subtype="XYZ",
        options={"ANIMATABLE"},
    )
    emissionAreaCutoutSize: bpy.props.FloatVectorProperty(
        default=(0, 0, 0), size=3, subtype="XYZ", options={"ANIMATABLE"}
    )
    emissionAreaRadius: bpy.props.FloatProperty(
        default=2, update=handleParticleSystemAreaSizeChange, options={"ANIMATABLE"}
    )
    ...
    emissionType: bpy.props.EnumProperty(
        options=set(), default="0", items=particleEmissionTypeList
    )
    randomizeWithParticleSizes2: bpy.props.BoolProperty(
        options=set(),
        default=False,
        description="Specifies if particles have random sizes",
    )
    particleSizes2: bpy.props.FloatVectorProperty(
        default=(1, 1, 1),
        size=3,
        subtype="XYZ",
        options={"ANIMATABLE"},
        description="The first two values are used to determine a random initial and final size for a particle",
    )
    randomizeWithRotationValues2: bpy.props.BoolProperty(options=set(), default=False)
    rotationValues2: bpy.props.FloatVectorProperty(
        default=(0, 0, 0), size=3, subtype="XYZ", options={"ANIMATABLE"}
    )
    randomizeWithColor2: bpy.props.BoolProperty(options=set(), default=False)
    initialColor2: bpy.props.FloatVectorProperty(
        default=(1, 1, 1, 1),
        min=0,
        max=1,
        size=4,
    ...
    middleColor2: bpy.props.FloatVectorProperty(
        default=(1, 1, 1, 1),
        min=0,
        max=1,
        size=4,
    ...
    finalColor2: bpy.props.FloatVectorProperty(
        default=(1, 1, 1, 0),
        min=0,
        max=1,
        size=4,
    ...
    phase1StartImageIndex: bpy.props.IntProperty(
        options=set(),
        default=0,
        min=0,
        max=255,
        subtype="UNSIGNED",
        description="Specifies the cell index shown at start of phase 1 when the image got divided into rows and collumns",
    )
    phase1EndImageIndex: bpy.props.IntProperty(
        options=set(),
        default=0,
        min=0,
        max=255,
        subtype="UNSIGNED",
        description="Specifies the cell index shown at end of phase 1 when the image got divided into rows and collumns",
    )
    phase2StartImageIndex: bpy.props.IntProperty(
        options=set(),
        default=0,
        min=0,
        max=255,
        subtype="UNSIGNED",
        description="Specifies the cell index shown at start of phase 2 when the image got divided into rows and collumns",
    )
    phase2EndImageIndex: bpy.props.IntProperty(
        options=set(),
        default=0,
        min=0,
        max=255,
        subtype="UNSIGNED",
        description="Specifies the cell index shown at end of phase 2 when the image got divided into rows and collumns",
    )
    relativePhase1Length: bpy.props.FloatProperty(
        options=set(),
        default=1,
        min=0,
        max=1,
        subtype="FACTOR",
        name="relative phase 1 length",
        description="A value of 0.4 means that 40% of the lifetime of the particle the phase 1 image animation will play",
    )
    numberOfColumns: bpy.props.IntProperty(
        options=set(),
        default=0,
        min=0,
        subtype="UNSIGNED",
        description="Specifies in how many columns the image gets divided",
    )
    numberOfRows: bpy.props.IntProperty(
        options=set(),
        default=0,
        min=0,
        subtype="UNSIGNED",
        description="Specifies in how many rows the image gets divided",
    )
    columnWidth: bpy.props.FloatProperty(
        options=set(),
        default=float("inf"),
        min=0,
        max=1,
        subtype="FACTOR",
        description="Specifies the width of one column, relative to an image with width 1",
    )
    rowHeight: bpy.props.FloatProperty(
        options=set(),
        default=float("inf"),
        min=0,
        max=1,
        subtype="FACTOR",
        description="Specifies the height of one row, relative to an image with height 1",
    )
    bounce: bpy.props.FloatProperty(
        ...
        min=0,
        max=1,
        description="Specifies the amount of velocity preserved when recoiling from a collision. 1 is elastic and 0 is sticky.",
    )
    friction: bpy.props.FloatProperty(
        ...
        min=0,
        max=1,
        description="Specifies the amount of velocity preserved when striking a collision surface. 1 is slippery and 0 is stuck.",
    )
    ...
    particleType: bpy.props.EnumProperty(
        options=set(), default="0", items=particleTypeList
    )
    lengthWidthRatio: bpy.props.FloatProperty(options=set(), default=1)
    localForceChannels: bpy.props.BoolVectorProperty(
        options=set(),
        default=tuple(16 * [False]),
        size=16,
        subtype="LAYER",
        description="If a model internal force shares a local force channel with a particle system then it affects it",
    )
    worldForceChannels: bpy.props.BoolVectorProperty(
        options=set(),
        default=tuple(16 * [False]),
        size=16,
        subtype="LAYER",
        description="If a force shares a force channel with a particle system then it affects it",
    )
    copies: bpy.props.CollectionProperty(options=set(), type=M3ParticleSystemCopy)
    copyIndex: bpy.props.IntProperty(
        options=set(), update=handleParticleSystemCopyIndexChanged
    )
    trailingParticlesName: bpy.props.StringProperty(options=set())
    trailingParticlesChance: bpy.props.FloatProperty(
        options=set(),
        default=0,
        min=0,
        max=1,
        subtype="FACTOR",
        name="trailingParticlesChance",
    )
    trailingParticlesRate: bpy.props.FloatProperty(
        default=10, name="trail. emiss. rate", options={"ANIMATABLE"}
    )
    noiseAmplitude: bpy.props.FloatProperty(options=set(), default=0)
    noiseFrequency: bpy.props.FloatProperty(options=set(), default=0)
    noiseCohesion: bpy.props.FloatProperty(
        options=set(),
        default=0,
        description="Prevents the particles from spreading to far from noise",
    )
    noiseEdge: bpy.props.FloatProperty(
        options=set(),
        default=0,
        min=0,
        max=0.5,
        subtype="FACTOR",
        description="The closer the value is to 0.5 the less noise will be at the start of particles life time",
    )
    ...
    windMultiplier: bpy.props.FloatProperty(
        options=set(), default=0, name="windMultiplier"
    )
...
class M3Ribbon(bpy.types.PropertyGroup):
    ...
    size: bpy.props.FloatVectorProperty(
        default=(1, 1, 1), size=3, subtype="XYZ", options={"ANIMATABLE"}
    )
    rotation: bpy.props.FloatVectorProperty(
        default=(1, 1, 1), size=3, subtype="XYZ", options={"ANIMATABLE"}
    )
    baseColoring: bpy.props.FloatVectorProperty(
        size=4, subtype="COLOR", min=0, max=1, options={"ANIMATABLE"}
    )
    centerColoring: bpy.props.FloatVectorProperty(
        size=4, subtype="COLOR", min=0, max=1, options={"ANIMATABLE"}
    )
    tipColoring: bpy.props.FloatVectorProperty(
        size=4, subtype="COLOR", min=0, max=1, options={"ANIMATABLE"}
    )
    sizeMiddleTime: bpy.props.FloatProperty(
        options=set(), default=1.0, min=0, max=1, subtype="FACTOR"
    )
    colorMiddleTime: bpy.props.FloatProperty(
        options=set(), default=1.0, min=0, max=1, subtype="FACTOR"
    )
    alphaMiddleTime: bpy.props.FloatProperty(
        options=set(), default=1.0, min=0, max=1, subtype="FACTOR"
    )
    rotationMiddleTime: bpy.props.FloatProperty(
        options=set(), default=1.0, min=0, max=1, subtype="FACTOR"
    )
    ...
    sizeSmoothing: bpy.props.EnumProperty(
        options=set(),
        default="0",
        items=animationSmoothTypeList,
        description="Determines the shape of the size curve based on the intial, middle , final and hold time value",
    )
    colorSmoothing: bpy.props.EnumProperty(
        options=set(),
        default="0",
        items=animationSmoothTypeList,
        description="Determines the shape of the color curve based on the intial, middle , final and hold time value",
    ...
    localForceChannels: bpy.props.BoolVectorProperty(
        ...
        size=16,
        subtype="LAYER",
        description="If a model internal force shares a local force channel with a ribbon then it affects it",
    )
    worldForceChannels: bpy.props.BoolVectorProperty(
        options=set(),
        default=tuple(16 * [False]),
        size=16,
        subtype="LAYER",
        description="If a force shares a force channel with a ribbon then it affects it",
    )
    ...
    endPointIndex: bpy.props.IntProperty(
        options=set(), default=-1, update=handleRibbonEndPointIndexChanged
    )
    lodReduce: bpy.props.EnumProperty(options=set(), default="0", items=shared.lodEnum)
    lodCut: bpy.props.EnumProperty(options=set(), default="0", items=shared.lodEnum)
    ...
    starRatio: bpy.props.FloatProperty(
        options=set(), default=0.5, min=0, max=1, subtype="FACTOR"
    )
...
class M3Force(bpy.types.PropertyGroup):
    ...
    type: bpy.props.EnumProperty(
        ...
        default="0",
        items=forceTypeList,
        update=handleForceNameOrTypeChange,
    )
    shape: bpy.props.EnumProperty(
        options=set(), default="0", items=forceShapeList, update=handleForceShapeUpdate
    )
    channels: bpy.props.BoolVectorProperty(
        options=set(),
        default=tuple(32 * [False]),
        size=32,
        subtype="LAYER",
        description="If a force shares a force channel with a particle system then it affects it",
    )
    ...
    width: bpy.props.FloatProperty(
        default=1, update=handleForceShapeUpdate, options={"ANIMATABLE"}, min=0.001
    )
    height: bpy.props.FloatProperty(
        default=0.05,
        update=handleForceShapeUpdate,
        options={"ANIMATABLE"},
        min=0.001,
        description="Value should be in the 0.001-0.999 range when force type is Cone Dome",
    )
    length: bpy.props.FloatProperty(
        default=0.05, update=handleForceShapeUpdate, options={"ANIMATABLE"}, min=0.001
    )
...
class M3PhysicsShape(bpy.types.PropertyGroup):
    ...
    offset: bpy.props.FloatVectorProperty(
        ...
        size=3,
        subtype="XYZ",
        update=handlePhysicsShapeUpdate,
    )
    rotationEuler: bpy.props.FloatVectorProperty(
        options=set(),
        default=(0, 0, 0),
        size=3,
        subtype="EULER",
    ...
    shape: bpy.props.EnumProperty(
        options=set(),
        default="0",
        items=physicsShapeTypeList,
        update=handlePhysicsShapeUpdate,
    )
    meshObjectName: bpy.props.StringProperty(options=set())
    # TODO: convex hull properties...
    size0: bpy.props.FloatProperty(
        options=set(), default=1, update=handlePhysicsShapeUpdate
    )
    size1: bpy.props.FloatProperty(
        options=set(), default=1, update=handlePhysicsShapeUpdate
    )
    size2: bpy.props.FloatProperty(
        options=set(), default=1, update=handlePhysicsShapeUpdate
    )
...
class M3RigidBody(bpy.types.PropertyGroup):
    ...
    friction: bpy.props.FloatProperty(
        options=set(), default=0.7, subtype="FACTOR", min=0, max=1
    )
    ...
    localForces: bpy.props.BoolVectorProperty(
        options=set(), default=tuple(16 * [False]), size=16, subtype="LAYER"
    )
...
class M3AttachmentPoint(bpy.types.PropertyGroup):
    name: bpy.props.StringProperty(
        options=set(), update=handleAttachmentPointNameOrTypeChange
    )
    boneName: bpy.props.StringProperty(options=set())
    bl_update: bpy.props.BoolProperty(options=set(), default=True)
    volumeType: bpy.props.EnumProperty(
        options=set(),
        default="-1",
        update=handleAttachmentVolumeTypeChange,
        items=attachmentVolumeTypeList,
    )
    volumeSize0: bpy.props.FloatProperty(
        options=set(), default=1, update=handleAttachmentVolumeSizeChange
    )
    volumeSize1: bpy.props.FloatProperty(
        options=set(), default=0, update=handleAttachmentVolumeSizeChange
    )
    volumeSize2: bpy.props.FloatProperty(
        options=set(), default=0, update=handleAttachmentVolumeSizeChange
    )
...
class M3SimpleGeometricShape(bpy.types.PropertyGroup):
    ...
    boneName: bpy.props.StringProperty(
        options=set(), update=handleGeometricShapeTypeOrBoneNameUpdate
    )
    shape: bpy.props.EnumProperty(
        options=set(),
        default="1",
        items=geometricShapeTypeList,
        update=handleGeometricShapeTypeOrBoneNameUpdate,
    )
    size0: bpy.props.FloatProperty(
        options=set(), default=1, update=handleGeometricShapeUpdate
    )
    size1: bpy.props.FloatProperty(
        options=set(), default=1, update=handleGeometricShapeUpdate
    )
    ...
    offset: bpy.props.FloatVectorProperty(
        ...
        size=3,
        subtype="XYZ",
        update=handleGeometricShapeUpdate,
    )
    rotationEuler: bpy.props.FloatVectorProperty(
        options=set(),
        default=(0, 0, 0),
...
class M3BoneVisiblityOptions(bpy.types.PropertyGroup):
    showFuzzyHitTests: bpy.props.BoolProperty(
        options=set(), default=True, update=handleFuzzyHitTestVisiblityUpdate
    )
    showTightHitTest: bpy.props.BoolProperty(
        options=set(), default=True, update=handleTightHitTestVisiblityUpdate
    )
    showAttachmentPoints: bpy.props.BoolProperty(
        options=set(), default=True, update=handleAttachmentPointVisibilityUpdate
    )
    showParticleSystems: bpy.props.BoolProperty(
        options=set(), default=True, update=handleParticleSystemsVisiblityUpdate
    )
    showRibbons: bpy.props.BoolProperty(
        options=set(), default=True, update=handleRibbonsVisiblityUpdate
    )
    showLights: bpy.props.BoolProperty(
        options=set(), default=True, update=handleLightsVisiblityUpdate
    )
    showForces: bpy.props.BoolProperty(
        options=set(), default=True, update=handleForcesVisiblityUpdate
    )
    showCameras: bpy.props.BoolProperty(
        options=set(), default=True, update=handleCamerasVisiblityUpdate
    )
    showPhysicsShapes: bpy.props.BoolProperty(
        options=set(), default=True, update=handlePhysicsShapeVisibilityUpdate
    )
    showProjections: bpy.props.BoolProperty(
        options=set(), default=True, update=handleProjectionVisibilityUpdate
    )
    showWarps: bpy.props.BoolProperty(
        options=set(), default=True, update=handleWarpVisibilityUpdate
    )
...
class M3Warp(bpy.types.PropertyGroup):
    ...
    radius: bpy.props.FloatProperty(
        default=1, min=0, update=handleWarpRadiusChange, options={"ANIMATABLE"}
    )
...
class M3Light(bpy.types.PropertyGroup):
    name: bpy.props.StringProperty(
        options=set(), default="", update=handleLightNameOrTypeChange
    )
    boneName: bpy.props.StringProperty(options=set())
    bl_update: bpy.props.BoolProperty(options=set(), default=True)
    lightType: bpy.props.EnumProperty(
        ...
        default="1",
        items=lightTypeList,
        update=handleLightNameOrTypeChange,
    )
    lightColor: bpy.props.FloatVectorProperty(
        default=(1, 1, 1), min=0, max=1, size=3, subtype="COLOR", options={"ANIMATABLE"}
    )
    lightIntensity: bpy.props.FloatProperty(default=1, options={"ANIMATABLE"})
    specColor: bpy.props.FloatVectorProperty(
        default=(1, 1, 1), min=0, max=1, size=3, subtype="COLOR", options={"ANIMATABLE"}
    )
    ...
    attenuationFar: bpy.props.FloatProperty(
        default=3, update=handleLightSizeChange, options={"ANIMATABLE"}
    )
    hotSpot: bpy.props.FloatProperty(default=1, options={"ANIMATABLE"})
    falloff: bpy.props.FloatProperty(
        default=1, update=handleLightSizeChange, options={"ANIMATABLE"}
    )
    lodCut: bpy.props.EnumProperty(options=set(), default="0", items=shared.lodEnum)
    shadowLodCut: bpy.props.EnumProperty(
        options=set(), default="0", items=shared.lodEnum
    )
...
class M3BillboardBehavior(bpy.types.PropertyGroup):
    ...
    billboardType: bpy.props.EnumProperty(
        options=set(), default="6", items=billboardBehaviorTypeList
    )
...
class M3TurretBehaviorPart(bpy.types.PropertyGroup):
    name: bpy.props.StringProperty(options=set())
    forwardX: bpy.props.FloatVectorProperty(
        options=set(), size=4, default=(0, -1, 0, 0)
    )
    ...
    yawWeight: bpy.props.FloatProperty(
        options=set(), min=0, max=1, default=1, subtype="FACTOR"
    )
    ...
    pitchWeight: bpy.props.FloatProperty(
        options=set(), min=0, max=1, default=1, subtype="FACTOR"
    )
...
class M3TurretBehavior(bpy.types.PropertyGroup):
    ...
    part_index: bpy.props.IntProperty(
        default=-1, update=handleTurretBehaviorPartIndexChanged
    )
...
class M3PhysicsJoint(bpy.types.PropertyGroup):
    ...
    offset1: bpy.props.FloatVectorProperty(
        options=set(), size=3, subtype="XYZ", update=handlePhysicsJointPivotDisplay
    )
    rotation1: bpy.props.FloatVectorProperty(
        options=set(),
        size=3,
        subtype="EULER",
        unit="ROTATION",
        update=handlePhysicsJointPivotDisplay,
    )
    offset2: bpy.props.FloatVectorProperty(
        options=set(), size=3, subtype="XYZ", update=handlePhysicsJointPivotDisplay
    )
    rotation2: bpy.props.FloatVectorProperty(
        options=set(),
        size=3,
        subtype="EULER",
        unit="ROTATION",
    ...
    dampingRatio: bpy.props.FloatProperty(
        options=set(), default=0.7, min=0, max=1, subtype="FACTOR"
    )
...
class CameraMenu(bpy.types.Menu):
    ...
    def draw(self, context):
        layout = self.layout
        draw_collection_op_generic(
            layout, "duplicate", "m3_cameras", "m3_camera_index", text="Duplicate"
        )
...
class RibbonsMenu(bpy.types.Menu):
    ...
    def draw(self, context):
        layout = self.layout
        draw_collection_op_generic(
            layout, "duplicate", "m3_ribbons", "m3_ribbon_index", text="Duplicate"
        )
...
class ForceMenu(bpy.types.Menu):
    ...
    def draw(self, context):
        layout = self.layout
        draw_collection_op_generic(
            layout, "duplicate", "m3_forces", "m3_force_index", text="Duplicate"
        )
...
class LightMenu(bpy.types.Menu):
    ...
    def draw(self, context):
        layout = self.layout
        draw_collection_op_generic(
            layout, "duplicate", "m3_lights", "m3_light_index", text="Duplicate"
        )
...
class WarpMenu(bpy.types.Menu):
    ...
    def draw(self, context):
        layout = self.layout
        draw_collection_op_generic(
            layout, "duplicate", "m3_warps", "m3_warp_index", text="Duplicate"
        )
...
class BoneVisibilityPanel(ContextScenePanel, bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        scene = context.scene
        layout.prop(
            scene.m3_bone_visiblity_options, "showFuzzyHitTests", text="Fuzzy Hit Tests"
        )
        layout.prop(
            scene.m3_bone_visiblity_options, "showTightHitTest", text="Tight Hit Test"
        )
        layout.prop(
            scene.m3_bone_visiblity_options,
        ...
        layout.prop(
            scene.m3_bone_visiblity_options, "showPhysicsShapes", text="Physics Shapes"
        )
        layout.prop(
            scene.m3_bone_visiblity_options, "showProjections", text="Projections"
        )
...
class AnimationSequencesPanel(ContextScenePanel, bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        shared.draw_collection_list(
            layout, "m3_animations", "m3_animation_index", "OBJECT_MT_M3_animations"
        )
...
class AnimationSequenceTransformationCollectionsPanel(
    ContextScenePanel, bpy.types.Panel
):
    ...
    def draw(self, context):
        ...
        shared.draw_collection_list(
            layout,
            path + "transformationCollections",
            path + "transformationCollectionIndex",
        )
        ...
        if index >= 0 and index < len(animation.transformationCollections):
            ...
            layout.prop(
                transformationCollection, "runsConcurrent", text="Runs Concurrent"
            )
...
def displayMaterialName(
    scene: bt.Scene, layout: bt.UILayout, materialReference: cm.M3Material
):
    layout = layout.split()
    material = cm.getMaterial(
        scene, materialReference.materialType, materialReference.materialIndex
    )
...
class MaterialReferencesPanel(ContextScenePanel, bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        col.template_list(
            "UI_UL_list",
            "m3_material_references",
            scene,
            "m3_material_references",
            scene,
            "m3_material_reference_index",
            rows=rows,
        )
        ...
        row.scale_y = 1.5
        row.operator("m3.prepare_3d_print", text="3D Prep (Classic)", icon="MODIFIER")
        row.operator("m3.prepare_3d_print_classic_enhanced", text="Classic+Detail", icon="MODIFIER")

        row = layout.row(align=True)
        row.scale_y = 1.5
        row.operator("m3.prepare_3d_print_geonodes", text="3D Prep (Modern)", icon="GEOMETRY_NODES")
        row.operator("m3.prepare_3d_print_geonodes_enhanced", text="Modern+Detail", icon="GEOMETRY_NODES")
...
class MaterialSelectionPanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        row = layout.row(align=True)
        row.scale_y = 2.0
        row.operator("m3.prepare_3d_print", text="Classic 3D Prep", icon="MODIFIER")
        row.operator("m3.prepare_3d_print_classic_enhanced", text="Classic+Detail", icon="MODIFIER")
        row = layout.row(align=True)
        row.scale_y = 2.0
        row.operator("m3.prepare_3d_print_geonodes", text="Modern 3D Prep (GeoNodes)", icon="GEOMETRY_NODES")
        row.operator("m3.prepare_3d_print_geonodes_enhanced", text="Modern+Detail", icon="GEOMETRY_NODES")
...
class M3_PT_debug_panel(ContextScenePanel, bpy.types.Panel):
    bl_idname = "SCENE_PT_m3_debug"
    bl_label = "M3 Addon Debug"
    bl_options = {'DEFAULT_CLOSED'}
    ...
    def draw(self, context):
        ...
        node_group_exists = "M3 3D Print Prep (Adaptive)" in bpy.data.node_groups
        status_icon = 'CHECKMARK' if node_group_exists else 'ERROR'
        layout.label(text=f"GeoNode Group: {status_icon}", icon=status_icon)
        
        layout.operator("m3.prepare_3d_print_geonodes", text="Force Build/Apply GeoNodes", icon='FILE_REFRESH')


def displayMaterialPropertiesUI(
    scene: bt.Scene, layout: bt.UILayout, materialReference
):
...
def displayMaterialPropertiesFlags(
    scene: bt.Scene, layout: bt.UILayout, materialReference
):
    materialType = materialReference.materialType
    materialIndex = materialReference.materialIndex
...
class MaterialPropertiesPanel(ContextScenePanel, bpy.types.Panel):
    ...
    def poll(cls, context):
        ...
        return scene and (
            mat.materialType != shared.stbMaterialTypeIndex
    ...
    def draw(self, context):
        scene = context.scene
        materialReference = scene.m3_material_references[
            scene.m3_material_reference_index
        ]
...
class ObjectMaterialPropertiesPanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        layout = self.layout
        displayMaterialPropertiesUI(
            scene,
            layout,
            context.scene.m3_material_references.get(
                context.object.data.m3_material_name
            ),
        )
...
class MaterialPropertiesFlagsPanel(bpy.types.Panel):
    ...
    def poll(cls, context):
        scene = context.scene
        return scene and (
            scene.m3_material_references[scene.m3_material_reference_index].materialType
            == shared.standardMaterialTypeIndex
            or scene.m3_material_references[
                scene.m3_material_reference_index
            ].materialType
            == shared.volumeNoiseMaterialTypeIndex
        )
...
class ObjectMaterialPropertiesFlagsPanel(bpy.types.Panel):
    ...
    def poll(cls, context):
        ...
        ob = context.object
        return ob and (
            scene.m3_material_references.get(ob.data.m3_material_name).materialType
            == shared.standardMaterialTypeIndex
            or scene.m3_material_references.get(ob.data.m3_material_name).materialType
            == shared.volumeNoiseMaterialTypeIndex
        )
...
class ObjectMaterialLayersPanel(bpy.types.Panel):
    ...
    def poll(cls, context):
        ...
        return (
            context.object
            and matRef is not None
            and matRef.materialType != shared.compositeMaterialTypeIndex
        )
    ...
    def draw(self, context):
        ...
        layout = self.layout
        displayMaterialLayersUI(
            scene,
            layout,
            scene.m3_material_references.get(context.object.data.m3_material_name),
        )
...
class MaterialLayersColorPanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        materialIndex = scene.m3_material_reference_index
        if not (
            materialIndex >= 0 and materialIndex < len(scene.m3_material_references)
        ):
...
class ObjectMaterialLayersUvPanel(bpy.types.Panel):
    ...
    def poll(cls, context):
        ...
        return (
            context.object
            and mat.layers[scene.m3_material_layer_index].colorEnabled == "0"
        )
...
class CameraPanel(ContextScenePanel, bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        shared.draw_collection_list(
            layout, "m3_cameras", "m3_camera_index", "OBJECT_MT_M3_cameras"
        )
...
class ParticleSystemsPropPanel(ContextScenePanel, bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        col.prop_search(
            particle_system,
            "trailingParticlesName",
            scene,
            "m3_particle_systems",
            text="Trailing Particles",
            icon="NONE",
        )
        sub = col.split(align=True)
...
class ParticleSystemsAreaPanel(ContextScenePanel, bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        sub = col.split(align=True)
        sub.active = (
            particle_system.cutoutEmissionArea
            and particle_system.emissionAreaType in emissionAreaTypesWithLength
        )
        sub.prop(particle_system, "emissionAreaCutoutSize", index=0, text="Length")
        sub = col.split(align=True)
        sub.active = (
            particle_system.cutoutEmissionArea
            and particle_system.emissionAreaType in emissionAreaTypesWithWidth
        )
        sub.prop(particle_system, "emissionAreaCutoutSize", index=1, text="Width")
        sub = col.split(align=True)
        sub.active = (
            particle_system.cutoutEmissionArea
            and particle_system.emissionAreaType == shared.emissionAreaTypeCuboid
        )
        ...
        sub.active = (
            particle_system.cutoutEmissionArea
            and particle_system.emissionAreaType in emissionAreaTypesWithRadius
        )
...
class ParticleSystemsMovementPanel(ContextScenePanel, bpy.types.Panel):
    ...
    def draw(self, context):
        scene = context.scene
        particle_system = context.scene.m3_particle_systems[
            scene.m3_particle_system_index
        ]
        layout = self.layout
...
class ParticleSystemsRotationPanel(ContextScenePanel, bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        col = row.column(align=True)
        col.prop(
            particle_system, "randomizeWithRotationValues2", text="Randomize With:"
        )
        sub = col.column(align=True)
...
class ParticleSystemsImageAnimPanel(ContextScenePanel, bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        layout.prop(
            particle_system, "relativePhase1Length", text="Relative Phase 1 Length"
        )
...
class ParticleSystemsFlagsPanel(ContextScenePanel, bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        col.prop(
            particle_system, "inheritEmissionParams", text="Inherit Emission Params"
        )
...
class RibbonsPanel(ContextScenePanel, bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        shared.draw_collection_list(
            layout, "m3_ribbons", "m3_ribbon_index", "OBJECT_MT_M3_ribbons"
        )
...
class RibbonColorPanel(ContextScenePanel, bpy.types.Panel):
    ...
    def poll(cls, context):
        return (
            context.scene
            and context.scene.m3_ribbon_index >= 0
            and context.scene.m3_ribbon_index < len(context.scene.m3_ribbons)
        )
...
class ForcePanel(ContextScenePanel, bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        shared.draw_collection_list(
            layout, "m3_forces", "m3_force_index", "OBJECT_MT_M3_forces"
        )
...
class PhysicsShapePanel(ContextScenePanel, bpy.types.Panel):
    ...
    def poll(cls, context):
        return (
            context.scene
            and context.scene.m3_rigid_body_index >= 0
            and len(context.scene.m3_rigid_bodies)
        )
    ...
    def draw(self, context):
        ...
        shared.draw_collection_list(
            layout, path + "physicsShapes", path + "physicsShapeIndex"
        )
...
class LightPanel(ContextScenePanel, bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        shared.draw_collection_list(
            layout, "m3_lights", "m3_light_index", "OBJECT_MT_M3_lights"
        )
...
class BillboardBehaviorPanel(ContextScenePanel, bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        shared.draw_collection_list(
            layout, "m3_billboard_behaviors", "m3_billboard_behavior_index"
        )

        if scene.m3_billboard_behavior_index >= 0:
            billboardBehavior = scene.m3_billboard_behaviors[
                scene.m3_billboard_behavior_index
            ]
            layout.separator()
...
class TurretBehaviorPanel(ContextScenePanel, bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        shared.draw_collection_list(
            layout, "m3_turret_behaviors", "m3_turret_behavior_index"
        )
...
class PhysicsJointPanel(ContextScenePanel, bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        layout.prop(
            scene, "m3_physics_joint_pivots", text="Display Physics Joint Pivot Helpers"
        )
        shared.draw_collection_list(
            layout, "m3_physics_joints", "m3_physics_joint_index"
        )
        ...
        if joint.jointType != shared.physicsJointWeld:
            ...
            if joint.jointType in [
                shared.physicsJointRevolute,
                shared.physicsJointCone,
            ]:
                brow = box.row()
...
class WarpPanel(ContextScenePanel, bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        shared.draw_collection_list(
            layout, "m3_warps", "m3_warp_index", "OBJECT_MT_M3_warps"
        )
...
class AttachmentPointsPanel(ContextScenePanel, bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        shared.draw_collection_list(
            layout, "m3_attachment_points", "m3_attachment_point_index"
        )
...
class FuzzyHitTestPanel(ContextScenePanel, bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        shared.draw_collection_list(
            layout, "m3_fuzzy_hit_tests", "m3_fuzzy_hit_test_index"
        )
...
class TightHitTestPanel(ContextScenePanel, bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if scene.m3_tight_hit_test.name == "":
            layout.operator(
                "m3.tight_hit_test_select_or_create_bone", text="Create Bone"
            )
        else:
            layout.operator(
                "m3.tight_hit_test_select_or_create_bone", text="Select Bone"
            )
...
class M3_MATERIALS_OT_add(bpy.types.Operator):
    ...
    def invoke(self, context, event):
        scene = context.scene
        self.materialName = shared.findUnusedPropItemName(
            propGroups=[scene.m3_material_references]
        )
        ...
        if referenceIndex >= 0:
            ...
            for particle_system in scene.m3_particle_systems:
                if particle_system.materialName == materialName:
                    self.report(
                        {"ERROR"},
                        'Can\'t delete: The particle system "%s" is using this material'
            ...
            for ribbon in scene.m3_ribbons:
                if ribbon.materialName == materialName:
                    self.report(
                        {"ERROR"},
                        'Can\'t delete: The ribbon "%s" is using this material'
                        % ribbon.name,
                    )
                    return {"CANCELLED"}
            for projection in scene.m3_projections:
                if projection.materialName == materialName:
                    self.report(
                        {"ERROR"},
                        'Can\'t delete: The projection "%s" is using this material'
                        % projection.name,
                    )
                    return {"CANCELLED"}
            for meshObject in shared.findMeshObjects(scene):
                ...
                if mesh.m3_material_name == materialName:
                    self.report(
                        {"ERROR"},
                        'Can\'t delete: The object "%s" (mesh "%s") is using this material.'
                        % (meshObject.name, mesh.name),
                    )
                    return {"CANCELLED"}

            for higherReferenceIndex in range(
                referenceIndex + 1, len(scene.m3_material_references)
            ):
                higherReference = scene.m3_material_references[higherReferenceIndex]
                material = cm.getMaterial(
                    scene, higherReference.materialType, higherReference.materialIndex
                )
                if material is not None:
            ...
            for otherReference in scene.m3_material_references:
                if (
                    otherReference.materialType == materialType
                    and otherReference.materialIndex > materialIndex
                ):
            ...
            for layerIndex in range(len(blenderMaterialsField[materialIndex].layers)):
                remove_m3_action_keyframes(
                    blenderMaterialsFieldName + "[{}].layers".format(materialIndex),
                    layerIndex,
                )
                for ii in range(
                    layerIndex, len(blenderMaterialsField[materialIndex].layers)
                ):
                    shift_m3_action_keyframes(
                        blenderMaterialsFieldName + "[{}].layers".format(materialIndex),
                        ii + 1,
                    )
            ...
            scene.m3_material_reference_index -= (
                1
                if scene.m3_material_reference_index
                == len(scene.m3_material_references)
                else 0
            )
        return {"FINISHED"}
...
class M3_MATERIALS_OT_duplicate(bpy.types.Operator):
    ...
    def invoke(self, context, event):
        ...
        matName = shared.findUnusedPropItemName(
            propGroups=[scene.m3_material_references], prefix=matRef.name
        )
        ...
        shared.copyBpyProps(
            newMat, mat, skip=["name", "layers", "sections", "materialReferenceIndex"]
        )
...
def copyCurrentActionOfObjectToM3Animation(scene, targetAnimation):
    sourceArmAction = bpy.data.actions.get(
        "Armature Object" + scene.m3_animations[scene.m3_animation_index].name
    )
    sourceScnAction = bpy.data.actions.get(
        "Scene" + scene.m3_animations[scene.m3_animation_index].name
    )
    ...
    for sourceAction, targetAction in (
        [sourceArmAction, newArmAction],
        [sourceScnAction, newScnAction],
    ):
        for sourceCurve in shared.action_fcurves(sourceAction):
            ...
            if sourceCurve.group is not None:
                ...
                targetCurve = shared.action_fcurves(targetAction).new(
                    path, index=arrayIndex, action_group=groupName
                )
            else:
                targetCurve = shared.action_fcurves(targetAction).new(
...
class M3_ANIMATIONS_OT_duplicate(bpy.types.Operator):
    ...
    def invoke(self, context, event):
        ...
        shared.copyBpyProps(
            newAnimation,
            animation,
            skip=["nameOld", "name", "transformationCollections"],
        )

        newAnimation.nameOld = uniqueNameFinder.findNameAndMarkAsUsedLike(
            animation.name
        )
        ...
        for obj in bpy.data.objects:
            if obj.type == "ARMATURE":
                ...
                for bone in armature.bones:
                    ...
                    rotLongAnimId = shared.getLongAnimIdOf(
                        objectId, animPathPrefix + "rotation_quaternion"
                    )
                    locLongAnimId = shared.getLongAnimIdOf(
                        objectId, animPathPrefix + "location"
                    )
                    scaleLongAnimId = shared.getLongAnimIdOf(
                        objectId, animPathPrefix + "scale"
                    )
                    if (
                        (rotLongAnimId in longAnimIds)
                        or (locLongAnimId in longAnimIds)
                        or (scaleLongAnimId in longAnimIds)
                    ):
                ...
                if obj.animation_data is not None:
                    ...
                    if action is not None:
                        for fcurve in shared.action_fcurves(action):
...
class M3_PARTICLE_SYSTEMS_OT_create_spawn_points_from_mesh(bpy.types.Operator):
    ...
    bl_description = (
        "Uses the vertices of the current mesh as spawn points for particles"
    )
    bl_options = {"UNDO"}
...
def remove_m3_action_keyframes(prefix, index):
    for action in [action for action in bpy.data.actions]:
        ...
        fcurves_coll = shared.action_fcurves(action)
        for fcurve in [
            fcurve
            for fcurve in fcurves_coll
            if prefix in fcurve.data_path and path in fcurve.data_path
        ]:
            fcurves_coll.remove(fcurve)
    ...
    for action in [action for action in bpy.data.actions]:
        ...
        for fcurve in [
            ...
            fcurve.data_path = fcurve.data_path.replace(
                path, "{}[{}]".format(prefix, index + offset)
            )
...
def swap_m3_action_keyframes(prefix, old, shift):
    for action in bpy.data.actions:
        ...
        fcurves = [fcurve for fcurve in fcurves_coll if path in fcurve.data_path]
        fcurves_shift = [
            fcurve for fcurve in fcurves_coll if path_shift in fcurve.data_path
        ]
...
def collection_item_add_get_name(item, prefix=""):
    ...
    if ".M3Animation'" in str(type(item)):
        return shared.findUnusedPropItemName(
            propGroups=[bpy.context.scene.m3_animations],
            suggestedNames=["Stand", "Walk", "Attack", "Spell"],
            prefix=prefix,
        )
    if ".M3AttachmentPoint'" in str(type(item)):
        return shared.findUnusedPropItemName(
            propGroups=[bpy.context.scene.m3_attachment_points],
            suggestedNames=["Origin", "Center", "Overhead", "Target"],
            prefix=prefix,
        )
    elif ".M3Camera'" in str(type(item)):
        return shared.findUnusedPropItemName(
            propGroups=[bpy.context.scene.m3_cameras],
            suggestedNames=["CameraPortrait", "CameraAvatar", "Camera"],
            prefix=prefix,
        )
    elif ".M3Force'" in str(type(item)):
        return shared.findUnusedPropItemName(
            propGroups=[bpy.context.scene.m3_forces], prefix=prefix
        )
    elif ".M3SimpleGeometricShape" in str(type(item)):
        return shared.findUnusedPropItemName(
            propGroups=[bpy.context.scene.m3_fuzzy_hit_tests], prefix="HitTestFuzzy"
        )
    elif ".M3Light'" in str(type(item)):
        return shared.findUnusedPropItemName(
            propGroups=[bpy.context.scene.m3_lights], prefix=prefix
        )
    elif ".M3ParticleSystem'" in str(type(item)) or ".M3ParticleSystemCopy'" in str(
        type(item)
    ):
        ...
        return shared.findUnusedPropItemName(
            propGroups=[bpy.context.scene.m3_projections], prefix=prefix
        )
    elif ".M3Ribbon'" in str(type(item)):
        return shared.findUnusedPropItemName(
            propGroups=[bpy.context.scene.m3_ribbons], prefix=prefix
        )
    elif ".M3RigidBody'" in str(type(item)):
        return shared.findUnusedPropItemName(
            propGroups=[bpy.context.scene.m3_rigid_bodies], prefix=prefix
        )
    elif ".M3TransformationCollection'" in str(type(item)):
        return shared.findUnusedPropItemName(
            propGroups=[
                bpy.context.scene.m3_animations[
                    bpy.context.scene.m3_animation_index
                ].transformationCollections
            ],
            suggestedNames=["full", "sub"],
            prefix=prefix,
        )
    elif ".M3PhysicsShape'" in str(type(item)):
        return shared.findUnusedPropItemName(
            propGroups=[
                bpy.context.scene.m3_rigid_bodies[
                    bpy.context.scene.m3_rigid_body_index
                ].physicsShapes
            ],
            prefix=prefix,
        )
        ...
        return shared.findUnusedPropItemName(
            propGroups=[bpy.context.scene.m3_warps], prefix=prefix
        )
...
def collection_item_remove_handle_bones(item):
    ...
    if ".M3Camera'" in str(type(item)):
        ...
        rigid_body = bpy.context.scene.m3_rigid_bodies[
            bpy.context.scene.m3_rigid_body_index
        ]
        shared.updateBoneShapeOfRigidBody(
            bpy.context.scene, rigid_body, rigid_body.name
        )
    elif ".M3SimpleGeometricShape'" in str(type(item)):
    ...
    def invoke(self, context, event):
        ...
        if ".M3SimpleGeometricShape'" in str(type(item)):
            ...
            item.name = (
                new_name
                if new_name
                else shared.findUnusedPropItemName(propGroups=[collection])
            )
...
class M3CollectionOpRemove(M3CollectionOpBase):
    ...
    def invoke(self, context, event):
        ...
        if ".M3Animation'" in str(type(item)):
            if len(collection) > 0:
                armAction = (
                    bpy.data.actions["Armature Object" + item.name]
                    if "Armature Object" + item.name in bpy.data.actions
                    else None
                )
                scnAction = (
                    bpy.data.actions["Scene" + item.name]
                    if "Scene" + item.name in bpy.data.actions
                    else None
                )
                ...
                bpy.context.scene.m3_animation_index -= (
                    1
                    if bpy.context.scene.m3_animation_index > 0 or len(collection) == 0
                    else 0
                )
                bpy.context.scene.m3_animation_index += (
                    1 if bpy.context.scene.m3_animation_index is len(collection) else 0
                )
        else:
            ...
            m3_ob_setter(
                ...
                collection_index - (1 if collection_index == len(collection) else 0),
            )
...
class M3CollectionOpMove(M3CollectionOpBase):
    ...
    def invoke(self, context, event):
        ...
        if (
            collection_index < len(collection) - self.shift
            and collection_index >= -self.shift
        ):
...
class M3CollectionOpDuplicate(M3CollectionOpBase):
    ...
    def invoke(self, context, event):
        ...
        item.name = (
            new_name
            if new_name
            else shared.findUnusedPropItemName(
                propGroups=[collection], prefix=item.name
            )
        )
        ...
        if ".M3ParticleSystem'" in str(type(item)):
            shared.copyBpyProps(
                ...
                skip=["name", "boneName", "copies", "spawnPoints"],
            )
        elif ".M3Ribbon'" in str(type(item)):
            shared.copyBpyProps(
                item,
                collection[collection_index],
                skip=["name", "boneName", "endPoints"],
            )
        elif ".M3RigidBody'" in str(type(item)):
            shared.copyBpyProps(
                item, collection[collection_index], skip=["name", "physicsShapes"]
            )
            ...
            shared.copyBpyProps(
                item, collection[collection_index], skip=["name", "boneName"]
            )
...
class ObjectSignOpAdd(bpy.types.Operator):
    ...
    bl_label = "Add faces"
    bl_description = (
        "Sets the normal sign of the vertices of all selected faces to positive"
    )
    bl_options = {"UNDO"}
...
class ObjectSignOpRemove(bpy.types.Operator):
    ...
    bl_description = (
        "Sets the normal sign of the vertices of all selected faces to negative"
    )
    bl_options = {"UNDO"}
...
class ObjectSignOpInvert(bpy.types.Operator):
    ...
    bl_description = (
        "Inverts the value of the sign inversion group for the selected faces."
    )
    bl_options = {"UNDO"}
...
def prepare_adaptive_3d_print():
    """
    Advanced 3D Print Prep: Smart Adaptive Subdivision.
    Updated for better robustness: searches for Normal Map nodes
    even if the texture is missing/unloaded.
    Created by MickeyKnox#1985
    """
    ...
    # 1. Identity Displacement/Normal Map source
    disp_image = None

    # Search through material slots to find normal/displacement map
    if not disp_image:
        for slot in obj.material_slots:
            ...
            # Strategy A: Look for Normal Map node connection
            nodes = mat.node_tree.nodes
            for node in nodes:
                if node.type == "NORMAL_MAP":
                    # Check what's connected to 'Color'
                    if node.inputs["Color"].links:
                        link = node.inputs["Color"].links[0]
                        if link.from_node.type == "TEX_IMAGE":
                            disp_image = link.from_node.image
                            if disp_image:
                                break

                # Strategy B: Fallback to keyword search in image name
                if node.type == "TEX_IMAGE" and node.image:
                    ...
                    if (
                        ...
                        disp_image = node.image
                        break
            if disp_image:
                break

    if not disp_image:
        print("\n[ERROR] Normal/Displacement map not found.")
        print(
            "FIX: Make sure textures are loaded OR select a Texture Node in the Shader Editor before running."
        )
        return

    print(f"Using {disp_image.name} for displacement.")
    ...
    # 2. Add Vertex Weight Edit
    vwe = obj.modifiers.new(name="M3_Smart_Mask", type="VERTEX_WEIGHT_EDIT")
    vwe.vertex_group = vg_name
    vwe.use_add = True
    vwe.add_threshold = 0.01

    weight_tex = bpy.data.textures.new("M3_Weight_Tex", type="IMAGE")
    weight_tex.image = disp_image
    # choose mask channel based on image channels (alpha if present, otherwise red)
    try:
        ch = getattr(weight_tex.image, 'channels', None)
    except Exception:
        ch = None
    ...
    subsurf = obj.modifiers.new(name="M3_Adaptive_Subsurf", type="SUBSURF")
    subsurf.subdivision_type = "SIMPLE"
    subsurf.levels = 4
    subsurf.render_levels = 4

    # 4. Add Displacement Modifier
    disp_tex = bpy.data.textures.new("M3_Disp_Tex_Final", type="IMAGE")
    disp_tex.image = disp_image

    disp = obj.modifiers.new(name="M3_Adaptive_Displace", type="DISPLACE")
    disp.texture = disp_tex
    disp.texture_coords = "UV"
    disp.vertex_group = vg_name
    disp.strength = 0.03  # Lowered default for safer first run
    disp.mid_level = 0.5

    # 5. Optimization Pass (Decimate)
    decimate = obj.modifiers.new(name="M3_Adaptive_Optimize", type="DECIMATE")
    decimate.ratio = 0.1
    decimate.vertex_group = vg_name
    decimate.invert_vertex_group = True

    print("\n--- Setup Complete! ---")
    print(f"Detail mask generated from texture: {disp_image.name}")
    print("If the model looks too 'noisy', increase mid-level or decrease strength.")


class M3_OT_prepare_3d_print_geonodes(bt.Operator):
    """
    Advanced 3D Print Prep (Geometry Nodes): Smart Adaptive Subdivision.
    Subdivides only where the relief map reveals details.
    Non-destructive and highly parametric.
    """

    bl_idname = "m3.prepare_3d_print_geonodes"
    bl_label = "Modern 3D Prep (GeoNodes)"
    ...
    def execute(self, context):
        ...
        from . import geonodes_3dprint
        success = geonodes_3dprint.apply_geonode_3dprint_prep(obj)
        if success:
            self.report({"INFO"}, "GeoNodes 3D Print Prep Applied!")
            return {"FINISHED"}
        else:
            self.report({"ERROR"}, "Failed to apply GeoNodes logic.")
            return {"CANCELLED"}


class M3_OT_prepare_3d_print(bpy.types.Operator):
    bl_idname = "m3.prepare_3d_print"
    bl_label = "Prepare for 3D Print"
    bl_description = (
        "Aplica mapas de normal em geometrias para impressão 3D (MickeyKnox#1985)"
    )
    bl_options = {"UNDO"}

    def execute(self, context):
        prepare_adaptive_3d_print()
        return {"FINISHED"}


class M3_OT_prepare_3d_print_classic_enhanced(bpy.types.Operator):
    """Classic Enhanced: improved modifier stack (Catmull-Clark, Edge Split, safer displacement)"""
    bl_idname = "m3.prepare_3d_print_classic_enhanced"
    bl_label = "Classic 3D Prep (Enhanced)"
    bl_options = {"UNDO"}
    ...
    def execute(self, context):
        ...
        from . import geonodes_3dprint
        success = geonodes_3dprint.apply_classic_enhanced_prep(obj)
        if success:
            self.report({"INFO"}, "Classic Enhanced 3D Prep applied.")
            return {"FINISHED"}
        else:
            self.report({"ERROR"}, "Failed to apply Classic Enhanced pipeline. Ensure a displacement/normal image exists on the material.")
            return {"CANCELLED"}


class M3_OT_prepare_3d_print_geonodes_enhanced(bt.Operator):
    """GeoNodes Enhanced: micro-detail preservation + adaptive subdivision"""
    bl_idname = "m3.prepare_3d_print_geonodes_enhanced"
    bl_label = "Modern 3D Prep (GeoNodes Enhanced)"
    bl_options = {"REGISTER", "UNDO"}
    ...
    def execute(self, context):
        ...
        from . import geonodes_3dprint
        success = geonodes_3dprint.apply_geonode_enhanced_prep(obj)
        if success:
            self.report({"INFO"}, "GeoNodes Enhanced 3D Prep Applied!")
            return {"FINISHED"}
        else:
            self.report({"ERROR"}, "Failed to apply GeoNodes Enhanced logic.")
            return {"CANCELLED"}


def menu_func_convertNormalMaps(self, context):
    self.layout.operator(
        M3_OT_conertBlenderToM3NormalMap.bl_idname,
        text="Convert Blender to M3 Normal Map",
    )
    self.layout.operator(
        M3_OT_conertM3ToBlenderNormalMap.bl_idname,
        text="Convert M3 to Blender Normal Map",
    )


def menu_func_import(self, context):
    self.layout.operator(
        ui.M3_OT_import.bl_idname, text="StarCraft 2 Model / Animation (.m3/.m3a)..."
    )
...
classes = (
    ...
    ui.M3_OT_export,
    ui.M3_OT_upscale_texture,
    ui.UpscalePanel,
    M3_OT_generateBlenderMaterails,
    M3_OT_conertBlenderToM3NormalMap,
    M3_OT_conertM3ToBlenderNormalMap,
    M3_OT_prepare_3d_print,
    M3_OT_prepare_3d_print_geonodes,
    M3_OT_prepare_3d_print_classic_enhanced,
    M3_OT_prepare_3d_print_geonodes_enhanced,
)
...
def register():
    ...
    bpy.types.Scene.m3_animation_index = bpy.props.IntProperty(
        update=handleAnimationSequenceIndexChange, default=-1
    )
    ...
    bpy.types.Scene.m3_material_references = bpy.props.CollectionProperty(
        type=cm.M3Material
    )
    bpy.types.Scene.m3_standard_materials = bpy.props.CollectionProperty(
        type=M3StandardMaterial
    )
    bpy.types.Scene.m3_displacement_materials = bpy.props.CollectionProperty(
        type=M3DisplacementMaterial
    )
    bpy.types.Scene.m3_composite_materials = bpy.props.CollectionProperty(
        type=M3CompositeMaterial
    )
    bpy.types.Scene.m3_terrain_materials = bpy.props.CollectionProperty(
        type=M3TerrainMaterial
    )
    bpy.types.Scene.m3_volume_materials = bpy.props.CollectionProperty(
        type=M3VolumeMaterial
    )
    bpy.types.Scene.m3_volume_noise_materials = bpy.props.CollectionProperty(
        type=M3VolumeNoiseMaterial
    )
    bpy.types.Scene.m3_creep_materials = bpy.props.CollectionProperty(
        type=M3CreepMaterial
    )
    bpy.types.Scene.m3_stb_materials = bpy.props.CollectionProperty(type=M3STBMaterial)
    bpy.types.Scene.m3_lens_flare_materials = bpy.props.CollectionProperty(
        type=M3LensFlareMaterial
    )
    bpy.types.Scene.m3_buffer_materials = bpy.props.CollectionProperty(
        type=M3MaterialNode
    )
    ...
    bpy.types.Scene.m3_camera_index = bpy.props.IntProperty(
        update=handleCameraIndexChanged, default=-1
    )
    bpy.types.Scene.m3_particle_systems = bpy.props.CollectionProperty(
        type=M3ParticleSystem
    )
    bpy.types.Scene.m3_particle_system_index = bpy.props.IntProperty(
        update=handleParticleSystemIndexChanged, default=-1
    )
    bpy.types.Scene.m3_ribbons = bpy.props.CollectionProperty(type=M3Ribbon)
    bpy.types.Scene.m3_ribbon_index = bpy.props.IntProperty(
        update=handleRibbonIndexChanged, default=-1
    )
    bpy.types.Scene.m3_forces = bpy.props.CollectionProperty(type=M3Force)
    bpy.types.Scene.m3_force_index = bpy.props.IntProperty(
        update=handleForceIndexChanged, default=-1
    )
    bpy.types.Scene.m3_rigid_bodies = bpy.props.CollectionProperty(type=M3RigidBody)
    bpy.types.Scene.m3_rigid_body_index = bpy.props.IntProperty(
        update=handleRigidBodyIndexChange, default=-1
    )
    bpy.types.Scene.m3_lights = bpy.props.CollectionProperty(type=M3Light)
    bpy.types.Scene.m3_light_index = bpy.props.IntProperty(
        update=handleLightIndexChanged, default=-1
    )
    bpy.types.Scene.m3_billboard_behaviors = bpy.props.CollectionProperty(
        type=M3BillboardBehavior
    )
    bpy.types.Scene.m3_billboard_behavior_index = bpy.props.IntProperty(
        update=handleBillboardBehaviorIndexChanged, default=-1
    )
    bpy.types.Scene.m3_ik_chains = bpy.props.CollectionProperty(
        type=M3InverseKinematicChain
    )
    bpy.types.Scene.m3_ik_chain_index = bpy.props.IntProperty(
        default=-1, update=handleIkChainIndexChanged
    )
    bpy.types.Scene.m3_turret_behaviors = bpy.props.CollectionProperty(
        type=M3TurretBehavior
    )
    bpy.types.Scene.m3_turret_behavior_index = bpy.props.IntProperty(default=-1)
    bpy.types.Scene.m3_physics_joint_pivots = bpy.props.BoolProperty(
        options=set(), default=True, update=handlePhysicsJointPivotDisplay
    )
    bpy.types.Scene.m3_physics_joints = bpy.props.CollectionProperty(
        type=M3PhysicsJoint
    )
    bpy.types.Scene.m3_physics_joint_index = bpy.props.IntProperty(
        default=-1, update=handlePhysicsJointIndexChanged
    )
    bpy.types.Scene.m3_projections = bpy.props.CollectionProperty(type=cm.M3Projection)
    bpy.types.Scene.m3_projection_index = bpy.props.IntProperty(
        update=cm.handleProjectionIndexChanged, default=-1
    )
    bpy.types.Scene.m3_warps = bpy.props.CollectionProperty(type=M3Warp)
    bpy.types.Scene.m3_warp_index = bpy.props.IntProperty(
        update=handleWarpIndexChanged, default=-1
    )
    bpy.types.Scene.m3_attachment_points = bpy.props.CollectionProperty(
        type=M3AttachmentPoint
    )
    bpy.types.Scene.m3_attachment_point_index = bpy.props.IntProperty(
        update=handleAttachmentPointIndexChanged, default=-1
    )
    bpy.types.Scene.m3_export_options = bpy.props.PointerProperty(
        type=cm.M3ExportOptions
    )
    bpy.types.Scene.m3_import_options = bpy.props.PointerProperty(
        type=cm.M3ImportOptions
    )
    bpy.types.Scene.m3_bone_visiblity_options = bpy.props.PointerProperty(
        type=M3BoneVisiblityOptions
    )
    ...
    bpy.types.Scene.m3_fuzzy_hit_tests = bpy.props.CollectionProperty(
        type=M3SimpleGeometricShape
    )
    bpy.types.Scene.m3_fuzzy_hit_test_index = bpy.props.IntProperty(
        update=handleFuzzyHitTestIndexChanged, default=-1
    )
    bpy.types.Scene.m3_tight_hit_test = bpy.props.PointerProperty(
        type=M3SimpleGeometricShape
    )
    bpy.types.Mesh.m3_material_name = bpy.props.StringProperty()
    bpy.types.Mesh.m3_physics_mesh = bpy.props.BoolProperty(
        default=False,
        description="Mark mesh to be used for physics shape only (not exported).",
    )
    ...
    bpy.types.Bone.m3_bind_scale = bpy.props.FloatVectorProperty(
        default=(1, 1, 1), size=3
    )
    bpy.types.EditBone.m3_bind_scale = bpy.props.FloatVectorProperty(
        default=(1, 1, 1), size=3
    )
